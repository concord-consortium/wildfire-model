...
const setVertexColor = (
  ...
  // way down the side of the model. Suppressing the river colour across that
...
const updateColors = (geometry: THREE.PlaneGeometry, simulation: SimulationModel, baseColorOnly = false) => {
  ...
  simulation.cells.forEach(cell => {
    setVertexColor(
      colArray, cell, simulation.gridWidth, simulation.gridHeight, simulation.config, debugColors, baseColorOnly,
      // Gated on the textured path so the untextured render stays byte-identical
      // to the unmodified app.
      baseColorOnly && isNearTerrainEdge(cell, simulation)
    );
  });
...
// immediate neighbours. The skirt FACE spans from a perimeter cell to the cell
...
const isNearTerrainEdge = (cell: Cell, simulation: SimulationModel) =>
  simulation.isTerrainEdge(cell.x, cell.y) ||
  simulation.isTerrainEdge(cell.x - 1, cell.y) ||
  simulation.isTerrainEdge(cell.x + 1, cell.y) ||
  simulation.isTerrainEdge(cell.x, cell.y - 1) ||
  simulation.isTerrainEdge(cell.x, cell.y + 1);

const updateVegetationWeights = ({ texture, data }: CellDataTexture, simulation: SimulationModel) => {
  data.fill(0);
  simulation.cells.forEach(cell => {
    ...
    // interior terrain.
    if (isNearTerrainEdge(cell, simulation)) return;
    data[cellDataIdx(cell, simulation.gridWidth) + cell.vegetation] = 255;
  });
  texture.needsUpdate = true;
};
...
const updateBurnState = ({ texture, data }: CellDataTexture, simulation: SimulationModel) => {
  ...
  simulation.cells.forEach(cell => {
    const idx = cellDataIdx(cell, simulation.gridWidth);
    let char = 0;
    let burning = 0;
    if (cell.fireState === FireState.Burnt) {
      ...
      const progress = (simulation.time - cell.ignitionTime) / cell.burnTime;
...
export const Terrain = observer(forwardRef<THREE.Mesh>(function WrappedComponent(props, ref) {
  ...
  // Exploratory textured terrain, off unless ?texturedTerrain=true. Until the
  // tiles finish loading — or if they fail to — `textured` stays false and the
  // original vertex-color path renders, so the sim is never blocked on them.
  const terrainTextures = useTerrainTextures(simulation.config.texturedTerrain);
  const textured = simulation.config.texturedTerrain && !!terrainTextures;
  ...
  useLayoutEffect(() => {
    if (geometryRef.current) {
      updateColors(geometryRef.current, simulation, textured);
    }
    // simulation.sparks.length retriggers the tpiDebug overlay as sparks are placed.
  }, [simulation, simulation.cellsStateFlag, simulation.sparks.length, textured]);
  ...
  useLayoutEffect(() => {
    if (cellData) {
      updateVegetationWeights(cellData.vegetationWeights, simulation);
    }
  }, [cellData, simulation, vegetationSignature, simulation.cellsElevationFlag]);
