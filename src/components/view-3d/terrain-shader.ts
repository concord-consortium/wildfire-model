...
import {
  BURNT_COLOR, BURN_INDEX_HIGH, BURN_INDEX_LOW, BURN_INDEX_MEDIUM, getTerrainColor
} from "./terrain-colors";
 ...
 * darkens — which is what washed the conifers out on the darkest zone color
 * while the same multiplier was fine on the pale drought colors.
 *
...
export const createTexturedTerrainMaterial = (
  ...
  const uniforms: TerrainShaderUniforms = {
    ...
    uSlopeFade: { value: new THREE.Vector2(...config.terrainTextureSlopeFade) },
    uTileRepeat: {
      value: new THREE.Vector2(
        config.modelWidth / config.terrainTextureTileFt,
        (config.modelWidth / config.terrainTextureTileFt) * (planeSize.y / planeSize.x)
      )
    },
    ...
    uHighlight: { value: config.terrainTextureHighlight },
    uMacroScale: { value: 5.0 },
    uMacroAmount: { value: config.terrainTextureMacroAmount },
    uEdgeNoiseScale: { value: config.terrainBurnEdgeNoiseScale },
    uEdgeSoftness: { value: config.terrainBurnEdgeSoftness },
    uFireOpacity: { value: 0.92 },
    uShowBurnIndex: { value: config.showBurnIndex ? 1 : 0 },
    uBurntColor: { value: srgb(BURNT_COLOR) },
    uBurningColor: { value: srgb([1, 0, 0]) },
  ...
  // Note: the slope fade uses dFdx/dFdy. Those are core in GLSL ES 3.00, which is
  // what three compiles to on the WebGL2 context it creates by default at r153.
