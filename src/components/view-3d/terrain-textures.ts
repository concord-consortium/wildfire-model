...
const TILE_DIR = "terrain-textures/";
...
const VEGETATION_TILE_FILES: Record<Vegetation, string> = {
...
export const useTerrainTextures = (enabled: boolean): TerrainTextures | null => {
  ...
  useEffect(() => {
    if (!enabled) return;
    let cancelled = false;
    let loaded: TerrainTextures | null = null;
    loadTerrainTextures(gl.capabilities.getMaxAnisotropy()).then(result => {
      ...
      // the sim stays fully usable and only loses the surface detail.
      // eslint-disable-next-line no-console
      console.error("[terrain-textures] disabled:", error);
    });
    return () => {
      cancelled = true;
      loaded?.vegetationTiles.dispose();
    };
  }, [enabled, gl]);
