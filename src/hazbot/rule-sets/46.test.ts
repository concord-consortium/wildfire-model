...
// SIMINIT defaults for tab 46: 3 zones (Mountains/Forest/Mild Drought,
// Foothills/Shrub/Medium Drought, Plains/Shrub/Medium Drought), wind 30 / 265.
const { defaults, changedZones, fireLine } = vars46;

function startReading(opts: Partial<WildfireReading> = {}): WildfireReading {
  return mkReading("SimulationStarted", opts.at ?? 100, { ...tab46.base, ...opts });
}
...
describe("ruleSet 46 — per-rule-set behavior sweep", () => {
  it("(a) empty readings → cat 1 (NOT ranSimulation)", () => {
    const e = makeWildfireEngine(ruleSet46, defaults);
    expect(matchAgainst(ruleSet46, e, [])).toBe(1);
  });
  it("(b) single match — ran with a changed variable → cat 2 (NOT DefaultVars)", () => {
    const e = makeWildfireEngine(ruleSet46, defaults);
    expect(matchAgainst(ruleSet46, e, [startReading({ zones: changedZones })])).toBe(2);
  });
  it("(c) multiple-true → highest wins — a plain run and a fireline run → cat 3 & 5 true → cat 5", () => {
    const e = makeWildfireEngine(ruleSet46, defaults);
    expect(matchAgainst(ruleSet46, e, [defaultNoFireline(), defaultWithFireline()])).toBe(5);
  });
  it("(d) stability — cat 4 holds across a later fireline run (no plain run appears)", () => {
    const e = makeWildfireEngine(ruleSet46, defaults);
    const r0 = defaultWithFireline(100);
    expect(matchAgainst(ruleSet46, e, [r0])).toBe(4);
    expect(matchAgainst(ruleSet46, e, [r0, defaultWithFireline(200)])).toBe(4);
  });
});

describe("ruleSet 46 — R9 per-category coverage", () => {
  const e = () => makeWildfireEngine(ruleSet46, defaults);
  it("cat 1 — no run", () => expect(matchAgainst(ruleSet46, e(), [])).toBe(1));
  it("cat 2 — ran with variables off default", () =>
    expect(matchAgainst(ruleSet46, e(), [startReading({ zones: changedZones })])).toBe(2));
  it("cat 3 — a plain default run, no fireline", () =>
    expect(matchAgainst(ruleSet46, e(), [defaultNoFireline()])).toBe(3));
  it("cat 4 — a default fireline run with no prior plain run", () =>
    expect(matchAgainst(ruleSet46, e(), [defaultWithFireline()])).toBe(4));
  it("cat 5 — a plain default run and a default fireline run", () =>
    expect(matchAgainst(ruleSet46, e(), [defaultNoFireline(), defaultWithFireline()])).toBe(5));
});

describe("ruleSet 46 — helitack-arm reachability (WM-28)", () => {
  ...
  it("cat 4 — a helitack-only run with no prior clean-baseline run (NOT X AND Y)", () => {
    expect(matchAgainst(ruleSet46, e(), [defaultWithHelitack(100)])).toBe(4);
  });
  it("cat 5 — a clean-baseline run plus a helitack-only run (X AND Y)", () => {
    expect(matchAgainst(ruleSet46, e(), [defaultNoFireline(), defaultWithHelitack()])).toBe(5);
  });
  it("cat 3 no longer over-matches — a helitack-only run is not classified cat 3", () => {
    ...
    expect(matchAgainst(ruleSet46, e(), [defaultWithHelitack(100)])).not.toBe(3);
  });
});

describe("ruleSet 46 — a paused run is one canonical run, not two (WM-28)", () => {
  ...
  it("start → pause → draw fire line → resume = one run → cat 4, not cat 5", () => {
    ...
    expect(matchAgainst(ruleSet46, e(), readings)).toBe(4);
  });
