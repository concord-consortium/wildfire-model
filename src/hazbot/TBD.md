...
and surface a `stub-warning` at load. Any category whose expression depends on
them is **unreachable** in production.
...
### `sawIntenseFire` — [wildfire/factor-variable-stubs.ts:8](wildfire/factor-variable-stubs.ts#L8)

- **Used by:** ruleset 34 Cat 4 and Cat 5.
- **Effect of stub:** ruleset 34 caps at Cat 3; intensity-comparison
  categories never fire.
- **What's needed:** the `SimulationStarted` / `SimulationEnded` /
  `SimulationStopped` payload's `outcome` field needs to carry an
  intensity-classification flag (red vs orange). Per the sheet's Details
  column: "Alert: new feature needed. The 'outcome' field could indicate
  whether high intensity (red, not orange) appeared significantly in the
  image generated by the simulation."
- **Substrate side:** `WildfireReading.outcome` is already typed as `unknown`
  ([wildfire/types.ts:12](wildfire/types.ts#L12)) and the `SimulationEnded`
  / `SimulationStopped` translators already pass `data.outcome` through
  ([wildfire/translate.ts:36](wildfire/translate.ts#L36)). The blocker is
  upstream: the simulation needs to compute and emit the intensity summary.
- **Open design questions:**
  - Threshold for "significant" intensity (e.g. fraction of burned cells
    that hit the high-intensity tier)?
  - Source: the existing burn-index UI uses some intensity tier already;
    can `outcome` reuse the same classifier?

---

## 3. Missing rule-sets entirely

Four LARA sequence pages embed the wildfire model but have no rule-set module.
They live in `EXCLUDED_TABS` in `scripts/extract-impl.js` because the source
sheet's rows are empty/TBD.

| Tab | Activity / Page | Preset | URL (in [localhost-urls.md](../../docs/hazbot-validation/localhost-urls.md)) |
|-----|-----------------|--------|------------------------------------------------------------------------------|
| 43 | 4 / 3 | `defaultTwoZone` | Listed in "Placeholder tabs" section |
| 45 | 4 / 5 | `townsThreeZone` | Listed in "Placeholder tabs" section |
| 47 | 4 / 7 | `dryTownsThreeZone` | Listed in "Placeholder tabs" section |
| 54 | 5 / 4 | `fiveTownsThreeZone` | Listed in "Placeholder tabs" section |

**To resolve:** sheet author fills tabs 43/45/47/54 in the source workbook with
categories + expressions + factor-variable defs, then:
1. Remove the tab id from `EXCLUDED_TABS` in `scripts/extract-impl.js`.
2. Re-run the extraction script.
3. Hand-author any new factor-variable or sim-prop impls the categories
   reference (`missing-impl` load failures will surface in the sidebar).
4. Add a per-rule-set test file (five-shape sweep — see existing
   `23.test.ts` / `24.test.ts` / `25.test.ts`).
5. Re-generate validation playbooks.
6. Move the row from "Placeholder tabs" to "Loadable rulesets" in
   `localhost-urls.md`.
