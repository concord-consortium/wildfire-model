...
### Authoring rule: every step has to read as an opener

A re-opened tour starts at the first step the student has not already satisfied, so any step can become the one they see first. No step after the first may open with a connective pointing back at the previous one ("First", "Second", "Third", "Then", "Now", "Next", "Also", "Finally"), because the step it refers to may never have been on screen. The `tour-map.test.ts` guard "no step after the first opens with a connective" fails on the common cases; its word list is deliberately tight, so a hit means read the line rather than that the line is certainly wrong.

The same applies to what a step asks for. A step may be anchored only to a control that is live at the point the tour can reach it: every tour opens on Restart or Clear All, both of which leave the run stopped, so a control that enables only mid-run (Fireline, Helitack) is dead for the whole tour. The "no step anchors a control that only enables while a run is in progress" guard covers this.
...
- **Selection-semantics changes**: bump. This covers any change to *which* string a given session is shown, whether the change is in how the existing expressions are evaluated (the windowed `category.current` that picks the category) or in how the category's feedback is picked once the category is known (the per-category feedback level, which serves a different string on a repeat click). The sheet is untouched in both cases, but what the student sees for a given history changes, which is exactly what the version marks. Bump in the commit that changes the semantics, not in a later docs commit: the repo deploys per-branch builds, so a mid-branch state is reachable, and a session logged from one would otherwise be indistinguishable from a session under the previous version's rules.
