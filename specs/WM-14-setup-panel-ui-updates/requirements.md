...
- [x] **Slider tick alignment** — designer sees the tick-mark alignment issue; **acceptable as-is** if it's the best we can do. _↪ relates to **Vegetation/Drought Type slider** marks + **Repositioning pass**; designer accepted, no action required unless easy._ ✅ Accepted as-is per designer; no change made.
...
- [x] **Top terrain images** — not interactive: no hover states, display at **100%**. _↪ ⚠️ reopened **Zone buttons** (the step-2 read-only `.fixed` thumbnails; "readonly states exist" was claimed but they still dim/hover)._ ✅ [zone-selector.scss](../../src/components/zone-selector.scss): `.fixed` excluded from the `:hover` rule and `.fixed .terrainImage { opacity: 100% }` (was the 50% edit-screen fade). Verified live: opacity 1, no hover response.
- [x] Check the **labels (font)** below the top terrain images. _↪ ⚠️ reopened **Repositioning pass** (step-2 recap `renderZoneTerrainTypeLabels` / TerrainSummary; see Line 16 detail)._ ✅ Already correct via the earlier `.terrainTypeLabel` fix — Mountains/Plains and the TerrainSummary captions render Roboto Condensed 13px #434343 (verified live).
- [x] **Vertically align** the drought labels to the drought icons. _↪ ⚠️ reopened **Repositioning pass** (TerrainSummary `.drought` icon + `.caption`)._ ✅ Drop was 5px below its 2-line caption (`padding-top: 6px` + box recentering); added `top: -5px` to `.drought` in [terrain-summary.scss](../../src/components/terrain-summary.scss). Verified live: center delta 0.
- [x] **Wind speed** — keep the value ("n MPH from / the NW") as **2 lines**; drop this section down a little. _↪ **Wind Direction control** (the two-line "0 MPH from\nthe N" text + section position) / **Wind Speed slider**._ ✅ Value already 2 lines; dropped the Wind Speed label/value/slider ~9px in [wind-circular-control.scss](../../src/components/wind-circular-control.scss) to match Zeplin (label/value left positions confirmed exact at 202/214).
- [ ] **Wind direction**: _↪ **Wind Direction control** (already `[ ]` open) + Deferred decisions #1 (interactive states) and #3 (dial face size)._
  - [x] Make it a little bigger (optional, no worries if not). _↪ Deferred #3 (WM-14 face is 79px per Zeplin; currently 72px — merge-time size reconciliation with WM-26)._ ✅ Dial enlarged 59→74px via a single `WIND_DIAL_SIZE` const in [wind-circular-control.tsx](../../src/components/wind-circular-control.tsx) (`WindDial` gained a `size` prop; default 59 keeps the model-display dial unchanged). Also recentered the dial + rotating wind symbol on a shared pivot.
  ...
  - [x] Note the hover/select of the **face** (something is off with its white outline currently) vs the hover/select of the **arrow**. _↪ Deferred #1 (per-face / per-arrow outline-opacity hover/select states)._ ✅ Investigated: the hover/select white halo is the SVG `#Highlight` disk sitting ~4px outside the `#Outline` ring — which matches Zeplin (its Face group is 79 with a 71 outline = ~4px margin too). Left as-spec; thinning it would mean shrinking the highlight in the asset. Per-face/per-arrow state split stays Deferred #1 (WM-26).
