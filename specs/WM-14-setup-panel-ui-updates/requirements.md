...
- [ ] **Terrains (zone backgrounds)** — new shortened terrain PNGs + river overlays; hover/select states. Re-export from artboard and replace in-place at **`src/public/terrain/{2,3}-zone-*.png`** (all 20 already exist there at the old `×136` height → new `×100`). The veg type icon over the thumbnail comes from `vegetationIcons` (already updated).
  - ✅ **Done:** the 15 plains/foothills/mountains backgrounds re-exported at `120×100` / `80×100` and committed.
  - ⚠️ **River overlays still TODO:** the 5 `*-zone-river-*.png` export from Zeplin as **trimmed thin strips** (e.g. `120×15`, `80×26`), not full-height. They won't drop into the current `.riverOverlay` (`height:100%; background-size:cover` would balloon the strip), and the trim drops the river's vertical position. Rework `.riverOverlay` to place the strip (likely bottom-anchored, `background-size:contain`/explicit offset) **when doing the terrain wiring + repositioning pass**, then re-export and commit. Old `×136` river PNGs left in place for now.
...
- [ ] **Wind Speed slider** — layout/position; **fix hit areas**; update thumb asset. Horizontal MUI Slider in wind-circular-control.tsx (`speedMarks`, classes `rail`/`mark`/`thumb`/`markLabel`). Partial: markLabel color #434343 + selected-mark **500** added. ⚠️ **13px marks deferred** — the slider is only ~37px wide (crammed in `.windText`), so 13px marks overlap ("0 102030"); kept at 10px with a TODO. Widening the slider + 13px + thumb + hit-area belong to the **wind-control layout pass** (couple with Wind Direction control below).
