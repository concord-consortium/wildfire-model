...
|---|---|---|---|
| 1 | Sam | The top category's repeat click uses category 100; 23/24's G6/H6 discarded as a fill-down (R4b) | **Code change.** The only one. A measured 9-line diff inside `ladder()`, recorded in the source spec so it does not have to be re-derived |
...
| 4 | Sam | Level 2 re-offers the coach-mark walk-through; level 3 is terminal (R1a, R4a) | One line in `normalizeFeedback` |
...
- **The sheet-side tidy-ups.** Nothing was typed into the workbook: clearing G6/H6 on tabs 23 and 24, the "Supression" fix, tab 34's category-100 narrative, and any explicit `[Show me]` tokens are all left to Trudi's next export, since editing our copy would trade the byte-for-byte re-extraction invariant for a silent divergence.
- **The Question 1 reversal.** The 9-line `ladder()` diff that would honor 23/24's Round columns on the top category is measured and recorded but not applied, pending Sam.
- **`testPathIgnorePatterns` scoped to exclude `<rootDir>/tmp/`.** Verified as the durable fix for the two locally-red suites left by this plan's spike artifacts (76 suites, 879 tests all green with it), but it is a `package.json` change outside this branch and was left as its own decision. CI is unaffected, since `tmp/` is gitignored.
