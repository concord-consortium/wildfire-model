...
The label is rendered at `simulation-info.tsx:52-54` as `` `${Math.round(scaledWind)} MPH from the ${degToCompass(direction)}` ``, styled by `.windText` in `simulation-info.scss:128` at a hardcoded `width: 68px` inside a `97 x 126` `.windContainer` with no `overflow: hidden`. Its siblings are `.windHeader` (a 14px Lato line with 6px top padding) and `.windDial` (a 59px square). All three are in normal flow, so an extra line of label pushes the dial down by one line box.
