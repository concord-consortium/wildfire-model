...
- **R9.** Amend `LOGGED-EVENTS.md`, which is the only file pinning the old ids in prose that exists on this branch. See R9a for its three edits. **The sibling specs are deliberately not in this requirement**; see Out of Scope.
- **R9a.** `LOGGED-EVENTS.md` needs **three** separate edits and none is a spec file. **(i)** Line 91 already reads *"It occurs only on tab 45"* on master today, in the `categoryId` note, and tab 45 becomes 44. **(ii)** Line 135 reads *"on rule-sets 42, 45, 47 and 54, whose middle categories carry no level 2"*. This one does not renumber: after this story 41, 44 and 46 all carry level 2 on their middle categories, so the clause is deleted rather than corrected, and 55 is not a substitute referent because it is excluded and carries no Round columns at all. **(iii)** WM-31 is scheduled ahead of this story and its R39 adds a `HazbotCoachMarkHiddenByRun` row that must *"name all seven, and split them"*, so that row ships carrying the seven-tour enumeration with the old ids and has to be corrected here to six tours split four and two: 41/2, 44/2, 46/2, 46/4 anchored on Start, and 44/3, 46/3 anchored on Fireline. **(iv)** A new subsection under `## Hazbot` recording the id renumber as a boundary in the logged data. `ruleSetId` is a join key on five payloads (`AnalysisEngineActivated`, `HazbotFeedbackShown`, `HazbotShowMeClicked`, `HazbotTourCompleted`, `HazbotTourDismissed`), and sessions either side of this deploy carry different values for the same page, which no consumer can recover after the fact. Follow the file's existing convention, `### <topic> (\`appRulesVersion\` N onward)`, as used at `:84`, `:98` and `:113`: state that 42, 45 and 47 became 41, 44 and 46 from `appRulesVersion` 8, and that **54 has no successor** because Act 5.5's Hazbot was removed by curriculum decision, so its series is closed rather than sparse. That last point is the whole of what the data consumers need and it is one paragraph, so no separate message is required; call it out in the PR description as well.
...
### The sheet already says Restart where WM-47 expects Clear All

Recorded here because a re-extract ships it, not because this story resolves it.

The category 2 `arrowText` on the new 44 and 46 was re-authored from *"First, **Reload** your model"* to *"First, **Restart** your model"*, and the matching `visualFeedback` was **not** touched: it still reads `1. Reload button outlined; coach mark points to Reload button`. `tour-map.tsx:130,135` still anchors `reload-button`. **Extracting as authored therefore ships a coach mark that rings Reload while the text says Restart.** Tab 41 was left as Reload, so the three tabs no longer agree with each other either.

This contradicts Trudi's own comment on WM-32 from 2026-08-18: *"As for the 'Reload' the model issue, we may be changing that to 'Clear All' that goes back to the default settings."* Read it as an edit made without WM-47's rename in view.

**The ring is the half that is right, measured rather than argued.** `simulation.ts:408` `restart()` resets the run (cells, time, fire-line markers, wind) and never touches zone vegetation, drought or `setupChanged`; `reload()` calls `restart()` and then adds `setupChanged = false`, `setInputParamsFromConfig()` and `populateCellsData()`. Confirmed live on `townsThreeZone`: zone vegetation `[1,1,1]` by default, `[0,1,1]` after changing zone 0, still `[0,1,1]` after `restart()`, back to `[1,1,1]` after `reload()`. Category 2 on 44 and 46 is `ranSimulation WITH NOT DefaultVars`, so it fires exactly because the student is off defaults, and **Restart cannot clear the condition the category exists to report**. Retargeting the anchor to `restart-button` would therefore make the coach mark self-consistent and useless; it is not an option.

**Nothing warns.** Running the real generator over the new extraction emits the mismatch with zero warnings: `tour-data-impl.js:119-120` compares the count of numbered lines in `visualFeedback` against the `arrowText` step count, both are 2, and prose naming the wrong button is invisible to it. This is the class of defect `docs/hazbot-update-workflow.md` section 6 warns about explicitly: a category can keep its step count and still retarget a coach mark, and every `tour-map.test.ts` invariant passes while the map points at the wrong control.

**The decision is WM-47's and is already open there**, as `### OPEN: What is Sam's wording for the three tour steps?` plus a resolved requirement that `arrowText` and `visualFeedback` be edited in the same pass. This story extracts as authored and changes no anchor.

### Three sheet edits arrive with the new content, not two

All three need a sheet edit, so they belong to WM-47's round-trip rather than a second export:

- **"Concord WIldfire"** with a capital I, in tab 46 category 4 Round 2.
- **Tab 41 still reading "Click to **Start** to run the model!"** where 44 and 46 both say "Click **Start**". This is the typo the sprint plan already folded into WM-47. The stray "to" sits between the ordinal and the bold run, which is why a plain grep for the phrase misses it.
- **`dsl-grammar.md:105`**, which reads *"the whole binary expression must be parenthesized (see the "45" sheet, for an example)"*. That file is generated from the workbook's README tab, and the scratch extraction's copy is byte-identical to the committed one, so Trudi's renumbering did not reach the sentence. R1's regeneration therefore reproduces a pointer to a tab that no longer exists. It cannot be fixed in the repo without hand-editing a `DO NOT EDIT` artifact, so it goes on the sheet with the other two. The example it points at is the `NOT (usedFireline AND usedHelitack) AND ...` expression on what is now **tab 44**, category 3.
...
- **The Reload versus Restart copy decision.** Extract the cells as authored. WM-47's spec already records the copy as Sam's call.
- **The three sheet edits** (two typos plus the `dsl-grammar.md` pointer), which need a sheet edit and belong to WM-47's round-trip.
...
- **Any change to a category expression, factor variable or sim-prop.** No expression changed in this sheet revision.
...
**Decision**: **A, verified rather than assumed.** The rename was rehearsed on the branch: `42/45/47.test.ts` renamed to `41/44/46.test.ts` with a purely mechanical substitution of `ruleSetNN` / `tabNN` / `varsNN` and the module path, and **all 32 assertions passed with no edit to any expectation**. WM-51's re-read discipline existed because expressions had changed; none changed here. The one thing the substitution missed was comment prose, which R4 now covers.
