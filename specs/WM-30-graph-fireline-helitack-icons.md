...
- **The icon needs no rounding to sit on its line, and none to stay crisp.** The plugin strokes the annotation line centered on the raw `scale.getPixelForValue(value)`; measured ink centers matched computed values inside 0.07px. The fractional `left` that produces does not soften the artwork, because these are SVG images rasterized at the draw position rather than bitmaps resampled onto a fractional offset. This property belongs to the SVG route specifically.
