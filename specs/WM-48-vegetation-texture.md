...
- **No per-cell loop in `terrain.tsx` reads `simulation.gridWidth` or `simulation.gridHeight`.** Both are `@computed`, and a computed read from a `useLayoutEffect` rather than from inside a reaction has no observers, so MobX re-evaluates it on every access. Fixing the near-edge predicate takes the textured burning median on a Chromebook from **82.2ms (100% of frames past the 66.5ms clamp) to 39.0ms (0%)**, measured during specing. Because `updateColors` is shared, the same hoist also fixes the untextured path, which master has been paying about 9.3ms/tick for. Re-run on the shipped branch the textured median is **36.8ms with 0% of frames over the clamp**; see Technical Notes for the full table.
...
**How it works.** Four 256 x 256 SVG tiles live in `src/public/terrain-textures/`, each carrying `#2A2A2A` glyph strokes on a transparent field, fetched by URL at runtime rather than imported: the webpack `.svg` rule pipes imported SVGs through SVGR (yielding a component, not a URL), and serving them statically lets an artist edit a tile and reload with no rebuild. They are rasterized to 512 x 512 through `<img>` plus a canvas prefilled with the `#808080` neutral (`TILE_FIELD` in `terrain-textures.ts`), then packed into the four channels of a single `THREE.DataTexture` in **Vegetation enum order**. The field is painted by whatever rasterizes a tile rather than drawn in the file, so the same tile serves as a CSS mask in the Setup panel; the raster is byte-identical either way. The packing exists because GLSL forbids indexing a sampler array by a per-fragment value. The texture is tagged `NoColorSpace` because the tiles are luminance data rather than color, with **128 as the neutral "unchanged" point**, and anisotropic filtering is applied because the terrain is viewed at a shallow angle.

**Three constraints found the hard way.** Tile scale is held as a ground distance (18,000 ft) and must be LARGE: detail below the pixel grid is averaged away by mipmapping, and since the tile mean is neutral by contract, a small tile does not get subtle, it vanishes. The one-cell skirt `fillTerrainEdges` creates smears planar UV badly, and the interpolated vertex normal cannot detect it because `computeVertexNormals` averages the faces meeting at each vertex, so the true geometric normal from `dFdx`/`dFdy` of local position is used. Those are core in GLSL ES 3.00, which three compiles to on the WebGL2 context it creates by default, but its WebGL1 fallback context needs `GL_OES_standard_derivatives`, and three emits that directive only for a material carrying `extensions.derivatives`; without it the program fails to link and the terrain mesh draws nothing. The material sets the flag. Vegetation weight doubles as the texture mask: the shader blends toward neutral by `(1 - weightSum)` rather than normalizing, so zeroing a cell's weights removes its glyphs with a soft one-cell fade, which is how rivers stay clear.
...
**Verified on the shipped branch, on the target Chromebook** (ANGLE Intel UHD Graphics 600 GLK 2, viewport 1241x529, DPR 1.10, 2 cores), production builds of `origin/master` and of this branch, `plainsTwoZone`, 4s idle then 10s of burning:

| run | burning median | fps | p95 | worst | frames over 66.5ms |
|---|---|---|---|---|---|
| master, plain | 32.6ms | 30.7 | 41.9ms | 47.8ms | 0% |
| branch, plain | **24.6ms** | **40.7** | 33.5ms | 40.5ms | 0% |
| branch, textured | 36.8ms | 27.2 | 45.9ms | 49.1ms | **0%** |

The requirement that the textured burning median land under 66.5ms passes with 29.7ms of headroom. The hoist's improvement to the default path is **8.0ms (24.5%)** here against the 8.2ms (22.3%) measured during specing; both absolutes sit about 4ms lower than the specing run, which is what thermal state looks like, and the delta reproducing to 0.2ms is what says the offset is not a measurement problem. Turning the key on costs 12.2ms against the new default and lands 4.2ms above where master sits today with no textures at all.
...
- **The textured path rebuilds the vertex colors on every tick.** Raised by Ethan in review. With `baseColorOnly` set, `updateColors` reads only the debug colors, `isRiver`, the near-edge mask, `isFireLineUnderConstruction` and `droughtLevel`, none of which a tick can change; but the effect is keyed on `cellsStateFlag`, so it rewrites an identical 38,400 x 4 buffer for the measured 7.88ms/tick, which is most of the 12.2ms the key costs. It is not a dependency-list fix: helitack drops reach `cell.droughtLevel` and fire-line construction reaches `isFireLineUnderConstruction`, and both signal only through `cellsStateFlag`, so skipping the work needs a dirty signal of its own. Deferred on the same grounds as `updateBurnState`, with 29.7ms of headroom against the clamp. Tracked as WM-56.
...
**Decision**: checked, and they do not collide. `fire-intensity-scale.tsx:3` is the **only** importer of those symbols anywhere in the repo, and the re-export existed precisely to keep it working. WM-52 moves where the component renders and requires the colors to keep deriving from `BURN_INDEX_*`, so it keeps importing through the same chain regardless of which lands first.

**Superseded in review**: with one importer, the re-export was a hop rather than a compatibility layer, so `fire-intensity-scale.tsx` now imports from `terrain-colors.ts` directly and the re-export is gone. WM-52 adjusts the relative path when it moves the file, exactly as it would have adjusted the old one.
...
**Confirmed on the shipped branch rather than on the probe builds**, same device and viewport: textured **36.8ms with 0% of frames over the clamp**, and the untextured default **32.6ms on master against 24.6ms here**. The figures above were taken on the prototype and probe builds, so this is the first measurement of the commits actually in the PR. The unfixed-prototype "before" of 82.2ms is not reproducible from this branch and stays attributed to specing, since it describes code that is not shipping.
...
**Made executable in review**, on Ethan's point that a threshold living only in prose is one the next story has to remember to re-measure. `bottom-bar-visuals.cy.ts` now asserts the bar does not overflow at the target device's 1241 x 529, with a rule-set loaded so the Hazbot button that sets the `.rightContainer` floor is present. That pins the viewport we have evidence for rather than inventing a minimum, and it fails on the change that pushes the row past the device rather than on every change to the row: the existing 576px `.mainContainer` assertion already forces WM-40 to re-measure.
...
**Decision**: **neither. The mask calls `simulation.isTerrainEdge` to build itself, rather than re-implementing it or moving onto the model.** The premise was wrong: the builder runs **once per grid**, not per tick, so it can afford the slow predicate, and duplicating it bought nothing. Measured over all 38,400 cells, every variant classifying the same 1,348 cells: 16.5ms via `simulation.isTerrainEdge`, 6.6ms reading `this.config.*`, 0.7ms with hoisted locals, 0.4ms as a mask lookup. The mask recovers 16.1 of 16.5ms, and paying the full 16.5ms once at grid-build time is free.

**Moved out of `terrain.tsx` in review**, into `near-edge-mask.ts` and a unit test, because the off-by-one below is exactly the failure a live count cannot catch: it compares two things that inherited the same fault. `buildNearEdgeMask` takes the grid dimensions, the flag and the predicate rather than the whole model, so the test drives it with a real `SimulationModel`'s `isTerrainEdge` and pins that the `gridHeight - 1` row is **not** masked. Both assertions were mutation-checked against the unbounded probe.
...
**The memo reads `fillTerrainEdges` rather than only listing it.** An early draft named it as a dependency without using it, because the body calls `simulation.isTerrainEdge`, which reads the flag internally; `exhaustive-deps` correctly flagged that, and silencing the rule was the wrong fix. The memo passes the flag, the two dimensions and the predicate into `buildNearEdgeMask`, so every dependency it lists is one it genuinely reads, and the short-circuit itself is true to `isTerrainEdge`'s own semantics.
