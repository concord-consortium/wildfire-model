...
Trudi's policy, settled in Slack on 2026-08-18: Hazbot waits. It can be clicked any time the model is not running, including while paused, and once the fire is done burning it can be clicked as many times as the student likes. During a run the button is visibly unavailable rather than silently unhelpful, and the robot stops blinking so it reads as asleep rather than idle.

## Requirements

Numbered R27 to R44a, continuing the numbering the interview used.

- **R27.** The Hazbot button is disabled while `simulation.simulationRunning` is true, and enabled whenever it is false. This puts paused on the enabled side (a Pause press and a Fire Line placement both stop the run), matches "before any run has been made", and re-enables automatically when a fire burns out.
...
- **`simulationRunning` is cleared from three places reached by four UI routes.** `simulation.stop()` serves both pause routes (the Pause press and Fire Line); `tick()` clears it on burnout. Restart is the fourth route and is a discard rather than a pause: it logs `SimulationEnded` plus `SimulationRestarted` rather than `SimulationStopped`. `handleHelitack` deliberately does not stop the run, so a Helitack drop is not a pause route. Worth knowing downstream: `SimulationRestarted` and `SimulationReloaded` are modifiers in `translate.ts` that close the open run window, so a mid-run Restart changes what the engine reports on the next click as well as re-enabling the button.
...
- **This was not a pre-existing bug.** `ui.showHazbotFeedback` had exactly three writers, all inside `hazbot-button.tsx`, and the only one firing during a tour also cleared `tourActive`. WM-31 introduces the first external writer, which is what makes the path reachable.
...
**Decision**: A, and the ask does not block the branch. Blocking would mean asking Trudi to re-state a policy she has stated twice and that is written into the description, while the only line contradicting it is one nobody is implementing. Two non-blocking messages ride along: one to Trudi carrying the description edit, the Fire Line consequence, and the 44/3 and 46/3 re-authoring ask; one to Sam warning that `HazbotTourCompleted` counts for six categories will fall.
...
**Decision**: Scope it and route the real fix to content. Out of Scope gains an entry naming the lost during-run instruction and the reopen-restarts-at-Restart consequence. The apparent escape hatch does not help: a Fire Line placement pauses the run and re-enables Hazbot, but the tour reopens at step 1, "First, **Restart** your model", which would throw away the run the student is intervening in. Re-authoring is Trudi's as owner of the feedback-table content.
